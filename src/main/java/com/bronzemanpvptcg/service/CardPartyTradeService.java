...
import com.bronzemanpvptcg.party.TcgTradeOfferCardDto;
...
import com.bronzemanpvptcg.ui.collectionalbum.AlbumInstanceTooltip;
...
public class CardPartyTradeService
{
	...
	public String sendTradeInvite(long recipientMemberId)
	{
		...
		BpvpTradeInvitePartyMessage m = new BpvpTradeInvitePartyMessage();
	...
	public String offerCard(String cardInstanceId)
	{
		...
		BpvpTradeOfferDeltaPartyMessage m = new BpvpTradeOfferDeltaPartyMessage();
		m.setTradeId(tradeId);
		m.setAdd(true);
	...
	public String setLocalReady(boolean ready)
	{
		...
		BpvpTradeReadyPartyMessage m = new BpvpTradeReadyPartyMessage();
		m.setTradeId(tradeId);
		m.setReady(ready);
	...
	public String cancelPendingOutboundInvite()
	{
		...
		BpvpTradeCancelPartyMessage m = new BpvpTradeCancelPartyMessage();
		m.setTradeId(tradeId);
		sendParty(m, null);
		notifyUi();
		return null;
	...
	private void handleInviteOnClientThread(BpvpTradeInvitePartyMessage msg)
	{
	...
	private void handleInviteAckOnClientThread(BpvpTradeInviteAckPartyMessage msg)
	{
		synchronized (lock)
		{
	...
	private void handleInviteResponseOnClientThread(BpvpTradeInviteResponsePartyMessage msg)
	{
		...
		if (parity != REJECT_NONE)
		{
			...
			BpvpTradeCancelPartyMessage cancel = new BpvpTradeCancelPartyMessage();
	...
	private void handleOfferDeltaOnClientThread(BpvpTradeOfferDeltaPartyMessage msg)
	{
		synchronized (lock)
		{
	...
	private void handleReadyOnClientThread(BpvpTradeReadyPartyMessage msg)
	{
		boolean shouldTryCommit = false;
		synchronized (lock)
		{
	...
	private void handleCancelOnClientThread(BpvpTradeCancelPartyMessage msg)
	{
	...
	private void handleCommitOnClientThread(BpvpTradeCommitPartyMessage msg)
	{
		TradeSession active;
	...
	private void trySendCommitAsLeader()
	{
		...
		BpvpTradeCommitPartyMessage m = new BpvpTradeCommitPartyMessage();
		m.setTradeId(tradeId);
	...
	private void sendInviteAck(String tradeId)
	{
		try
		{
			BpvpTradeInviteAckPartyMessage ack = new BpvpTradeInviteAckPartyMessage();
		...
		Boolean localDebugLogging)
	{
		try
		{
			BpvpTradeInviteResponsePartyMessage r = new BpvpTradeInviteResponsePartyMessage();
	...
	private static RewardTuningState tuningFromInvite(BpvpTradeInvitePartyMessage msg)
	{
	...
	private static RewardTuningState tuningFromInviteResponse(BpvpTradeInviteResponsePartyMessage msg)
	{
		return RewardTuningState.mergeSerialized(
			msg.getFoilChancePercent(),
			msg.getKillCreditMultiplier(),
	...
	private static RewardTuningState tuningFromCommit(BpvpTradeCommitPartyMessage msg)
	{
		return RewardTuningState.mergeSerialized(
			msg.getFoilChancePercent(),
			msg.getKillCreditMultiplier(),
